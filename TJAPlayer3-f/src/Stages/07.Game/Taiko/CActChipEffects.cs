...
internal class CActChipEffects : CActivity
{
    ...
    public override int On進行描画()
    {
        for (int i = 0; i < 128; i++)
        {
            ...
            TJAPlayer3.Tx.Notes.Opacity = 255 - (int)Math.Min((500.0 * (si.ct進行.n現在の値 / (double)si.ct進行.n終了値)), 255.0);
            if (TJAPlayer3.Tx.Notes_White != null)
                TJAPlayer3.Tx.Notes_White.Opacity = (int)Math.Min((500.0 * (si.ct進行.n現在の値 / (double)si.ct進行.n終了値)), 255.0);//2020.05.15 Mr-Ojii ノーツを白くするために追加。
            switch (si.nプレイヤー)
            {
                case 0:
                    ...
                    TJAPlayer3.Tx.Notes.t2D拡大率考慮描画(TJAPlayer3.app.Device, CTexture.RefPnt.Center, TJAPlayer3.app.Skin.SkinConfig.Game.Effect.FlyingNotes.EndPointX[0], TJAPlayer3.app.Skin.SkinConfig.Game.Effect.FlyingNotes.EndPointY[0], new Rectangle(si.Lane * 130, 0, 130, 130));
                    if (TJAPlayer3.Tx.Notes_White != null)
                        TJAPlayer3.Tx.Notes_White.t2D拡大率考慮描画(TJAPlayer3.app.Device, CTexture.RefPnt.Center, TJAPlayer3.app.Skin.SkinConfig.Game.Effect.FlyingNotes.EndPointX[0], TJAPlayer3.app.Skin.SkinConfig.Game.Effect.FlyingNotes.EndPointY[0], new Rectangle(si.Lane * 130, 0, 130, 130));
                    break;
                case 1:
                    ...
                    TJAPlayer3.Tx.Notes.t2D拡大率考慮描画(TJAPlayer3.app.Device, CTexture.RefPnt.Center, TJAPlayer3.app.Skin.SkinConfig.Game.Effect.FlyingNotes.EndPointX[1], TJAPlayer3.app.Skin.SkinConfig.Game.Effect.FlyingNotes.EndPointY[1], new Rectangle(si.Lane * 130, 0, 130, 130));
                    if (TJAPlayer3.Tx.Notes_White != null)
                        TJAPlayer3.Tx.Notes_White.t2D拡大率考慮描画(TJAPlayer3.app.Device, CTexture.RefPnt.Center, TJAPlayer3.app.Skin.SkinConfig.Game.Effect.FlyingNotes.EndPointX[1], TJAPlayer3.app.Skin.SkinConfig.Game.Effect.FlyingNotes.EndPointY[1], new Rectangle(si.Lane * 130, 0, 130, 130));
                    break;
            }
