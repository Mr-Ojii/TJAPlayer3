...
``Sounds/Taiko/SElist.csv``に行を追加し、その音色の説明を記載することで追加が可能となります。  
(一応、csvの記述方法にのっとり、カンマ区切りでも可)

#### 注意点
* フォルダだけ作り、フォルダ内に音声ファイルが存在しない場合でも、1音色としてカウントされます。
* 音声ファイルが欠けている場合、``Sounds/Taiko``内の音声を使用します。
...
詳しくは``About_SkinConfig.ini.txt``で説明いたします。


## GenreConfig
ジャンルの種類を記述します。  
記述方法はcsv形式です。  
改行ごとに違うジャンル・,区切りで横に並べた文字列は同じジャンルとして認識されます。
...
この例の記述では"VOCALOID"と"ボーカロイド"は同じジャンルとして認識され、  
"VOCALOID"と"ナムコオリジナル"は違うジャンルとして認識されます。  
このファイルの記述内容はBox.def,テクスチャのジャンル数に反映されます。
