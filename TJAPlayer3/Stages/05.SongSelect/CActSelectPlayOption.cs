...
namespace TJAPlayer3
{
	internal class CActSelectPlayOption : CActivity
	{
		...
		private List<ItemTextureList> MakeItemnameTexture(List<CItemBase> cItemBases)
		{
			...
			{
				CTexture NameTexture = TJAPlayer3.tCreateTexture(this.Font.DrawPrivateFont(cItemBases[i].strName, Color.White, Color.Black, TJAPlayer3.Skin.Font_Edge_Ratio));
				...
				if (cItemBases[i].eItemType == CItemBase.EItemType.List) {
					...
					for (int index = 0; index < ((CItemList)cItemBases[i]).list項目値.Count; index++) {
						ListTexture[index] = TJAPlayer3.tCreateTexture(this.Font.DrawPrivateFont(((CItemList)cItemBases[i]).list項目値[index], Color.White, Color.Black, TJAPlayer3.Skin.Font_Edge_Ratio));
					}
				}
				else
				{
